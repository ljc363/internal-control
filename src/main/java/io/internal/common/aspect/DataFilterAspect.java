...
public class DataFilterAspect {
    ...
    @Pointcut("@annotation(io.internal.common.annotation.DataFilter)")
    ...
    private String getSQLFilter(SysUserEntity user, JoinPoint point){
        ...
        if(roleIdList.size() > 0){
            List<Long> userPositionIdList = sysRolePositionService.qureyPositionList(roleIdList.toArray(new Long[roleIdList.size()]));
            deptIdList.addAll(userPositionIdList);
        }

        //用户岗位ID列表
        if(dataFilter.subPosition()){
            List<Long> subDeptIdList = sysPositionServier.getSubDeptIdList(user.getPositionId());
        ...
        if(deptIdList.size() > 0){
            sqlFilter.append(tableAlias).append(dataFilter.positionId()).append(" in(").append(StringUtils.join(deptIdList, ",")).append(")");
        }
